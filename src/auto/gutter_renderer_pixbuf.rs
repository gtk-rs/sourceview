...
impl<O: IsA<GutterRendererPixbuf>> GutterRendererPixbufExt for O {
    ...
    fn get_stock_id(&self) -> Option<GString> {
        unsafe {
            from_glib_none(gtk_source_sys::gtk_source_gutter_renderer_pixbuf_get_stock_id(self.as_ref().to_glib_none().0))
        }
    }
    ...
    fn set_stock_id(&self, stock_id: Option<&str>) {
        unsafe {
            gtk_source_sys::gtk_source_gutter_renderer_pixbuf_set_stock_id(self.as_ref().to_glib_none().0, stock_id.to_glib_none().0);
        }
    }
    ...
    fn connect_property_stock_id_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_stock_id_trampoline<P, F: Fn(&P) + 'static>(this: *mut gtk_source_sys::GtkSourceGutterRendererPixbuf, _param_spec: glib_sys::gpointer, f: glib_sys::gpointer)
            where P: IsA<GutterRendererPixbuf>
        {
            let f: &F = &*(f as *const F);
            f(&GutterRendererPixbuf::from_glib_borrow(this).unsafe_cast())
